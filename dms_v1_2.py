...
"""# Rider Allocation using Optimization (New Cost Function)"""

def optimize(rider,order1,fulltime,parttime,maxorders=3,thresholddist=10,thresholdorder=2):
  costs=[]
  for i in range(len(rider)):
    riderdist=[]
    riderloc=rider['Rider Latitude-Longitude'].loc[i]
    riderqueue=rider['Number of Orders in Queue'].loc[i]
    ridername=rider['Name'].loc[i]
    for j in range(len(order1)):
      shoploc=order1['Shop Latitude-Longitude'].loc[j]
      if ridername in fulltime:
        totaldist= ((float)(200*(distance(riderloc,shoploc)/thresholddist))+ (float)(100*(riderqueue/thresholdorder)))
      elif ridername in parttime:
        totaldist= ((float)(100*(distance(riderloc,shoploc)/thresholddist))+ (float)(200*(riderqueue/thresholdorder)))
...
def call():
  ...
  rdrs=rider['Name'].tolist()
  random.shuffle(rdrs)
  cut = random.randint(0, len(rdrs))
  full_time_riders= rdrs[:cut]
  contract_riders= rdrs[cut:]
  print('Rider Table')
  display(rider)
  print('Order Table')
  display(order1)
  print('In-House Riders:')
  print(full_time_riders)
  print('Contractual Riders:')
  print(contract_riders)
  ...
  #print(len(selected_orders))
  optimized_data=optimize(rider_available,selected_orders,full_time_riders,contract_riders,maxorders)
...
def task_order_assign_tsp(riderindex,assign,order,rider):
  ...
  while not routing.IsEnd(index):
    ...
    dropofflatlong.append(latlongs[manager.IndexToNode(index)])
  ...
  #print(dropoffqueue)
  return pickupqueue,dropoffqueue,dropofflatlong,route_distance
...
for i in range(len(rider_assigned)):
  ...
  p,d,l,dist=task_order_assign_tsp(rider_assigned[i],assign,order,rider)
  pick.append(p)
  drop.append(d)
  ll.append(l)
  tot=tot+dist
...
routings['Latitude Longitudes']=ll
routings.to_csv('routes_withlatlong.csv',index=False)
print(tot)